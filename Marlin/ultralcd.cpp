...
//static float arm_c22, arm_s22, arm_k12, arm_k22, arm_psi2, arm_theta2, arm_dcal_x2, arm_dcal_y2=0;
...
static void lcd_move_e()
{
    ...
    if (lcdDrawUpdate)
    {
        lcd_implementation_drawedit(PSTR("Retract"), ftostr31(current_position[E_AXIS]));
    }
    if (LCD_CLICKED)
    {
        lcd_quick_feedback();
...
static void lcd_move_straight()
{
  if (encoderPosition != 0)
  {
   ...
   encoderPosition = 0;
    ...
    if (lcdDrawUpdate)
    {
        lcd_implementation_drawedit(PSTR("X-Y Pos"), ftostr31(current_position[X_AXIS]));
    }
    if (LCD_CLICKED)
    {
        lcd_quick_feedback();
...
static void lcd_move_menu_axis()
{
    ...
    MENU_ITEM(submenu, "Move X-axis", lcd_move_x);
    MENU_ITEM(submenu, "Move Y-axis", lcd_move_y);
    MENU_ITEM(submenu, "Move Straight", lcd_move_straight);
    if (move_menu_scale < 10.0)
    {
        MENU_ITEM(submenu, "Move Z-axis", lcd_move_z);
        MENU_ITEM(submenu, "Retract", lcd_move_e);
    }
    END_MENU();
}
...
static void lcd_control_motion_menu()
{
    ...
    MENU_ITEM_EDIT(bool, "Endstop abort", &abort_on_endstop_hit); // da tradurre
#endif
    END_MENU();
}
